...
# on_command_reply(Reply) -> autogenerated queue
# service.send(reply_to=True) -> autogenerated queue
# service.send(reply_to='my-command-reply') -> queue 'my-command-reply'
#   check how reply to works
...
def test_command_bus_reply_listener(
    ...
    def callback(msg: CommandWithMessage) -> str:
        q.put(msg)
    ...
    listeners = [
        command_bus.listener("replies", "reply_to_me", callback),
    ...
    with run_consumer(consumer):
        command_bus.send("replies", "reply_to_me", {"number": "one"})

        message = q.get(block=True, timeout=1)
        assert isinstance(message, CommandWithMessage)
        assert message.command == {"number": "one"}
